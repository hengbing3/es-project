...
public class PictureServiceImpl implements PictureService {
    @Override
    public Page<PictureEntity> queryPicturePage(PictureQueryParam pictureQueryParam) {
        int current = (pictureQueryParam.getCurrentPage() - 1) * pictureQueryParam.getPageSize();
        String url = String.format("https://www.bing.com/images/search?q=%s&first=%s", pictureQueryParam.getSearchText(), current);
        Document doc = null;
        try {
        ...
        for (Element element : elements) {
            if (pictureEntities.size() >= pictureQueryParam.getPageSize() ) {
                break;
            }
            // 获取图片地址
        ...
        Page<PictureEntity> entityPage = new Page<>(pictureQueryParam.getCurrentPage(), pictureQueryParam.getPageSize());
